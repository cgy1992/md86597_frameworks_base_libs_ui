                    ...
                    if (res == sizeof(iev)) {
                        ...
                        *outScancode = iev.code;
                        if (iev.type == EV_KEY) {
							...
							//2011-02-23 Eric Fang-Cheng Liu
							//Use LOGD for debugging.
                            //LOGV("iev.code=%d outKeycode=%d outFlags=0x%08x err=%d\n",
                            LOGD("iev.code=%d outKeycode=%d outFlags=0x%08x err=%d\n",
...
// [BEGIN] skyviia modify: pull in mouse input event!
                if (test_bit(BTN_LEFT, key_bitmask) && test_bit(BTN_RIGHT, key_bitmask)){
					LOGV("device->classes CLASS_MOUSE...");
                    device->classes |= CLASS_MOUSE;
                	}
                else{
// [END]
                LOGV("device->classes CLASS_TRACKBALL...");
    ...
    //2011-04-14 Eric Fang-Cheng Liu
    //Messages For the ABS device class.
	//LOGI("ABS MAP\n");
    for (int i=0; i<((ABS_MAX+7)/8); i++) {
		//Messages For the ABS device class.
        //LOGI("[%d]: 0x%02x\n", i, abs_bitmask[i]);
		//2011-04-14 Eric Fang-Cheng Liu
		//Reset the abs bitmask for Mantis 3500 when some abnormal state.
		if(abs_bitmask[i] == 0xFF)
		{
			abs_bitmask[i] = 0x00;
		}
		//Messages For the ABS device class.
		//LOGI("After Tunner, [%d]: 0x%02x\n", i, abs_bitmask[i]);
    }
...
// [BEGIN] skyviia modify: for debug key layout mapping!
        LOGI("keylayoutFilename='%s'\n", keylayoutFilename);
// [END]
        bool defaultKeymap = false;
        if (access(keylayoutFilename, R_OK)) {
            snprintf(keylayoutFilename, sizeof(keylayoutFilename),
                     "%s/usr/keylayout/%s", root, "default.kl");
            defaultKeymap = true;
        }
